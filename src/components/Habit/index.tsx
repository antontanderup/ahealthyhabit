      ...
      <View style={styles.content}>
        ...
        {habit.goals && habit.goals.length > 0 ? (
          habit.goals.map((goal, index, goals) => {
            const isDisabled = goals[index - 1] != null && longestStreak / goals[index - 1] < 1;
          ...
          <Pressable onPress={onEditOpen} style={[styles.chip, styles.chipNotReached]}>
            <Text style={[styles.chipText, styles.chipTextDefault]}>{t('addGoals')}</Text>
          </Pressable>
        )}
      </View>
    </View>
...
const useDefaultStyles = createUseStyles(theme => ({
  ...
  titleInfo: {flex: 1},
  title: {fontSize: 16, fontWeight: '600', color: theme.onSurface},
  subtitle: {fontSize: 14, marginTop: 2, color: theme.onSurfaceVariant},
  cardActions: {flexDirection: 'row', alignItems: 'center', marginRight: 4},
  iconButton: {padding: 8},
  iconButtonPressed: {opacity: 0.5},
  content: {flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 16},
  chip: {
    flexDirection: 'row',
    alignItems: 'center',
    borderRadius: 50,
  ...
  chipWithAvatar: {paddingLeft: 6},
  chipGoalReached: {backgroundColor: theme.primaryContainer},
  chipNotReached: {backgroundColor: theme.surfaceContainerHigh},
  ...
  chipTextGoalReached: {color: theme.onPrimaryContainer},
  chipTextDefault: {color: theme.onSurfaceVariant},
}));
